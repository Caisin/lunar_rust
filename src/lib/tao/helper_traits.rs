...
impl RefHelper for TaoRef {
    ...
    fn get_year_in_chinese(&self) -> String {
        ...
        let mut s = vec![];
        y.iter().for_each(|yy| {
            s.push(lunar_util::NUMBER()[yy.to_string().parse::<i64>().unwrap() as usize].clone());
        });
        s.join("")
    }
    ...
    fn is_day_an_wu(&self) -> bool {
        util::tao_util::AN_WU()[self.get_month() as usize - 1] == self.get_lunar().get_day_zhi()
    }
