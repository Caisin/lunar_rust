...
impl RefHelper for SolarMonthRef {
    ...
    fn get_days(&self) -> Vec<SolarRef> {
        ...
        days.push(d.clone());
        let days_of_month = solar_util::get().get_days_of_month(self.get_year(), self.get_month());
    ...
    fn get_weeks(&self, start: i64) -> Vec<SolarWeekRef> {
        let mut weeks = vec![];
        let mut week = SolarWeek::from_ymd(self.get_year(), self.get_month(), 1, start);
        loop {
            weeks.push(week.clone());
            week = week.next(1, false);
            let first_day = week.get_first_day();
            if first_day.get_year() > self.get_year() || first_day.get_month() > self.get_month() {
                break;
            }
        }
        weeks
