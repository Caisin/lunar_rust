...
use crate::{
    ...
    util::{
        lunar_util::{
            GAN, HE_GAN_5, HE_ZHI_6, JIA_ZI, LunarUtil, NAYIN, SHI_SHEN, WU_XING_GAN, WU_XING_ZHI,
            ZHI, ZHI_HIDE_GAN,
        },
...
impl EightCharRefHelper for Arc<Mutex<EightChar>> {
    ...
    ///
    fn get_tai_yuan(&self) -> String {
        let mut gan_index = self.get_lunar().get_month_gan_index_exact() + 1;
        if gan_index >= 10 {
            gan_index = gan_index - 10;
        }
        let mut zhi_index = self.get_lunar().get_month_zhi_index_exact() + 3;
        if zhi_index >= 12 {
    ...
    fn get_ming_gong(&self) -> String {
        ...
        let mut jia_zi_index =
            LunarUtil::get_jia_zi_index(&self.get_month()) - (month_zhi_index - zhi_index) as i64;
        if jia_zi_index >= 60 {
    ...
    fn get_shen_gong(&self) -> String {
        ...
        let mut jia_zi_index = LunarUtil::get_jia_zi_index(&self.get_month())
