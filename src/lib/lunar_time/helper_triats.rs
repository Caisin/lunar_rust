use crate::{
    ...
    util::{
        ...
        lunar_util::{
            CHONG, CHONG_GAN, CHONG_GAN_TIE, GAN, LunarUtil, NAYIN, POSITION_CAI, POSITION_DESC,
            POSITION_FU, POSITION_FU_2, POSITION_XI, POSITION_YANG_GUI, POSITION_YIN_GUI, SHA,
            SHENGXIAO, TIAN_SHEN, TIAN_SHEN_TYPE, TIAN_SHEN_TYPE_LUCK, ZHI, ZHI_TIAN_SHEN_OFFSET,
        },
    },
};
...
impl RefHelper for LunarTimeRef {
    ...
    fn get_sheng_xiao(&self) -> String {
        SHENGXIAO()[self.as_locked_ref().__zhi_index as usize + 1].to_string()
    }

    fn get_position_xi(&self) -> String {
        POSITION_XI()[self.as_locked_ref().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_yang_gui(&self) -> String {
        POSITION_YANG_GUI()[self.as_locked_ref().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_yin_gui(&self) -> String {
        POSITION_YIN_GUI()[self.as_locked_ref().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_fu(&self, sect: Option<i64>) -> String {
        ...
        position_fu()[self.as_locked_ref().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_cai(&self) -> String {
        POSITION_CAI()[self.as_locked_ref().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_chong_gan_tie(&self) -> String {
        CHONG_GAN_TIE()[self.as_locked_ref().__gan_index as usize].to_string()
    }

    fn get_chong_sheng_xiao(&self) -> String {
        let chong = self.get_chong();
        let zhi = ZHI();
        let found = zhi.iter().enumerate().find(|z| z.1.to_string() == chong);
        match found {
    ...
    fn get_yi(&self) -> Vec<String> {
        LunarUtil::get_time_yi(
            &{ self.as_locked_ref().__lunar.clone() }.get_day_in_gan_zhi_exact(),
            &self.get_gan_zhi(),
        )
    }
