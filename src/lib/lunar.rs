...
use crate::{
    ...
    util::{
        lunar_util::{BASE_MONTH_ZHI_INDEX, LunarUtil, POSITION_GAN, POSITION_TAI_SUI_YEAR},
...
pub fn from_ymd(lunar_year: i64, lunar_month: i64, lunar_day: i64) -> LunarRef {
...
pub fn from_solar(solar: &SolarRef) -> LunarRef {
    ...
    for m in ly.get_months().iter() {
        let days = solar.subtract(solar::from_julian_day(m.get_first_julian_day()));
...
impl Lunar {
    fn __new(
        ...
        assert!(
            lunar_day <= days,
            "only {days} days in lunar_year {lunar_year} lunar_month {lunar_month} "
        );
        ...
        let m = m.unwrap();
        let noon = solar::from_julian_day(m.get_first_julian_day() + (lunar_day - 1) as f64);
...
impl Lunar {
    fn __compute_jie_qi(y: Arc<Mutex<LunarYear>>) -> (HashMap<String, SolarRef>, Vec<String>) {
        ...
        JIE_QI_IN_USE().iter().enumerate().for_each(|(i, jq)| {
            ...
            jie_qi.insert(name.clone(), solar::from_julian_day(julian_days[i]));
    ...
    fn __compute_day(solar: &SolarRef, hour: &i64, minute: &i64) -> (i64, i64, i64, i64, i64, i64) {
        ...
        let hm = NaiveTime::from_hms_opt(hour.clone() as u32, minute.clone() as u32, 0).unwrap();
    ...
    fn __compute_time(hour: &i64, minute: &i64, day_gan_index_exact: &i64) -> (i64, i64) {
        ...
        let __time_gan_index = (day_gan_index_exact % 5 * 2 + time_zhi_index) % 10;
    ...
    fn __get_month_position_tai_sui(month_zhi_index: i64, month_gan_index: i64) -> String {
    ...
    fn __get_day_position_tai_sui(day_in_gan_zhi: &str, year_zhi_index: i64) -> String {
        let to_find = [
            "甲子,乙丑,丙寅,丁卯,戊辰,已巳"
                .split(",")
                .collect::<Vec<_>>(),
        ...
        let zipped = to_find.iter().zip(response.iter()).collect::<Vec<_>>();
    ...
    fn __convert_jie_qi(name: &str) -> String {
        let __from = [
            "DONG_ZHI", "DA_HAN", "XIAO_HAN", "LI_CHUN", "DA_XUE", "YU_SHUI", "JING_ZHE",
        ];
    ...
    fn __get_month_nine_star(year_zhi_index: i64, month_zhi_index: i64) -> NineStarRef {
