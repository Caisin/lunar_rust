...
use crate::{
    ...
    util::{
        foto_util::{DAY_ZHAI_GUAN_YIN, FESTIVAL, OTHER_FESTIVAL, get_XIU},
        locked_ref_trait::LockRef,
        lunar_util::{ANIMAL, GONG, NUMBER, SHOU, XIU_LUCK, XIU_SONG, ZHENG},
    },
};
...
impl RefHelper for FotoRef {
    ...
    fn is_day_zhai_six(&self) -> bool {
        match self.get_day() {
            d if 8 == d || 14 == d || 15 == d || 23 == d || 29 == d || 30 == d => true,
            28 => {
                let m = lunar_month::from_ym(self.get_lunar().get_year(), self.get_month());
                m.get_day_count() != 30
            }
            _ => false,
        }
