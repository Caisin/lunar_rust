...
pub fn from_bazi(
    ...
    // ---------------------------
    let mut offset_year =
        LunarUtil::get_jia_zi_index(&today.get_lunar().get_year_in_gan_zhi_exact())
    ...
    let zipped_chunks = zippeds.chunks(10).map(|z| z.to_vec()).collect::<Vec<_>>();
...
impl Solar {
    fn __cache() -> Arc<Mutex<HashMap<String, SolarRef>>> {
        static __SOLAR_CACHE: LazyLock<Arc<Mutex<HashMap<String, SolarRef>>>> =
    ...
    fn __new(
        ...
        let key = format!("{}-{}-{} {}:{}:{}", year, month, day, hour, minute, second);
