use super::{JIE_QI, Lunar, LunarRef, LunarRefHelper, helper_traits::__LunarRefHelper};
use crate::{
    ...
    util::{
        lunar_util::{
            ANIMAL, CHONG, CHONG_GAN, CHONG_GAN_TIE, DAY, FESTIVAL, GAN, GONG, HOU, LIU_YAO, LU,
            LunarUtil, MONTH, NAYIN, NUMBER, OTHER_FESTIVAL, PENG_ZU_GAN, PENG_ZU_ZHI,
            POSITION_CAI, POSITION_DESC, POSITION_FU, POSITION_FU_2, POSITION_TAI_DAY,
            POSITION_TAI_MONTH, POSITION_TAI_SUI_YEAR, POSITION_XI, POSITION_YANG_GUI,
            POSITION_YIN_GUI, SEASON, SHA, SHENGXIAO, SHOU, TIAN_SHEN, TIAN_SHEN_TYPE,
            TIAN_SHEN_TYPE_LUCK, WU_HOU, XIU, XIU_LUCK, XIU_SONG, YUE_XIANG, ZHENG, ZHI,
...
impl LunarRefHelper for LunarRef {
    ...
    fn get_year_gan(&self) -> String {
        GAN()[self.lock().unwrap().__year_gan_index as usize + 1].to_string()
    }

    fn get_year_gan_by_li_chun(&self) -> String {
        GAN()[self.lock().unwrap().__year_gan_index_by_li_chun as usize + 1].to_string()
    }

    fn get_year_gan_exact(&self) -> String {
        GAN()[self.lock().unwrap().__year_gan_index_exact as usize + 1].to_string()
    }

    fn get_year_zhi(&self) -> String {
        ZHI()[self.lock().unwrap().__year_zhi_index as usize + 1].to_string()
    }

    fn get_year_zhi_by_li_chun(&self) -> String {
        ZHI()[self.lock().unwrap().__year_zhi_index_by_li_chun as usize + 1].to_string()
    }

    fn get_year_zhi_exact(&self) -> String {
        ZHI()[self.lock().unwrap().__year_zhi_index_exact as usize + 1].to_string()
    }
    ...
    fn get_year_in_gan_zhi_exact(&self) -> String {
        format!("{}{}", self.get_year_gan_exact(), self.get_year_zhi_exact())
    }

    fn get_month_gan(&self) -> String {
        GAN()[self.lock().unwrap().__month_gan_index as usize + 1].to_string()
    }

    fn get_month_gan_exact(&self) -> String {
        GAN()[self.lock().unwrap().__month_gan_index_exact as usize + 1].to_string()
    }

    fn get_month_zhi(&self) -> String {
        ZHI()[self.lock().unwrap().__month_zhi_index as usize + 1].to_string()
    }

    fn get_month_zhi_exact(&self) -> String {
        ZHI()[self.lock().unwrap().__month_zhi_index_exact as usize + 1].to_string()
    }
    ...
    fn get_day_gan_exact(&self) -> String {
        GAN()[self.lock().unwrap().__day_gan_index_exact as usize + 1].to_string()
    }

    fn get_day_gan_exact2(&self) -> String {
        GAN()[self.lock().unwrap().__day_gan_index_exact2 as usize + 1].to_string()
    }
    ...
    fn get_day_zhi_exact(&self) -> String {
        ZHI()[self.lock().unwrap().__day_zhi_index_exact as usize + 1].to_string()
    }

    fn get_day_zhi_exact2(&self) -> String {
        ZHI()[self.lock().unwrap().__day_zhi_index_exact2 as usize + 1].to_string()
    }
    ...
    fn get_day_in_gan_zhi_exact2(&self) -> String {
        format!("{}{}", self.get_day_gan_exact2(), self.get_day_zhi_exact2())
    }

    fn get_time_gan(&self) -> String {
        GAN()[self.lock().unwrap().__time_gan_index as usize + 1].to_string()
    }

    fn get_time_zhi(&self) -> String {
        ZHI()[self.lock().unwrap().__time_zhi_index as usize + 1].to_string()
    }
    ...
    fn get_year_sheng_xiao(&self) -> String {
        SHENGXIAO()[self.lock().unwrap().__year_zhi_index as usize + 1].to_string()
    }

    fn get_year_sheng_xiao_by_li_chun(&self) -> String {
        SHENGXIAO()[self.lock().unwrap().__year_zhi_index_by_li_chun as usize + 1].to_string()
    }

    fn get_year_sheng_xiao_exact(&self) -> String {
        SHENGXIAO()[self.lock().unwrap().__year_zhi_index_exact as usize + 1].to_string()
    }

    fn get_month_sheng_xiao(&self) -> String {
        SHENGXIAO()[self.lock().unwrap().__month_zhi_index as usize + 1].to_string()
    }

    fn get_month_sheng_xiao_exact(&self) -> String {
        SHENGXIAO()[self.lock().unwrap().__month_zhi_index_exact as usize + 1].to_string()
    }

    fn get_day_sheng_xiao(&self) -> String {
        SHENGXIAO()[self.lock().unwrap().__day_zhi_index as usize + 1].to_string()
    }

    fn get_time_sheng_xiao(&self) -> String {
        SHENGXIAO()[self.lock().unwrap().__time_zhi_index as usize + 1].to_string()
    }
    ...
    fn get_year_in_chinese(&self) -> String {
        ...
        for c in y.chars().collect::<Vec<_>>().iter() {
            s = format!("{}{}", s, NUMBER()[c.to_string().parse::<usize>().unwrap()]);
        }
        s
    }
    ...
    fn get_peng_zu_gan(&self) -> String {
        PENG_ZU_GAN()[self.lock().unwrap().__day_gan_index as usize + 1].to_string()
    }

    fn get_peng_zu_zhi(&self) -> String {
        PENG_ZU_ZHI()[self.lock().unwrap().__day_zhi_index as usize + 1].to_string()
    }
    ...
    fn get_day_position_xi(&self) -> String {
        POSITION_XI()[self.lock().unwrap().__day_gan_index as usize + 1].to_string()
    }
    ...
    fn get_day_position_yang_gui(&self) -> String {
        POSITION_YANG_GUI()[self.lock().unwrap().__day_gan_index as usize + 1].to_string()
    }
    ...
    fn get_day_position_yin_gui(&self) -> String {
        POSITION_YIN_GUI()[self.lock().unwrap().__day_gan_index as usize + 1].to_string()
    }
    ...
    fn get_day_position_fu(&self, sect: Option<i64>) -> String {
        ...
        position_fu()[self.lock().unwrap().__day_gan_index as usize + 1].to_string()
    }
    ...
    fn get_day_position_cai(&self) -> String {
        POSITION_CAI()[self.lock().unwrap().__day_gan_index as usize + 1].to_string()
    }
    ...
    fn get_year_position_tai_sui_desc(&self, sect: Option<i64>) -> String {
        POSITION_DESC()
    ...
    fn get_month_position_tai_sui(&self, sect: Option<i64>) -> String {
        ...
        Lunar::__get_month_position_tai_sui(month_zhi_index, month_gan_index)
    }

    fn get_month_position_tai_sui_desc(&self, sect: Option<i64>) -> String {
        POSITION_DESC()
    ...
    fn get_day_position_tai_sui(&self, sect: Option<i64>) -> String {
        let (__year_zhi_index, __year_zhi_index_exact, __year_zhi_index_by_li_chun) = {
            let s = self.lock().unwrap();
            (
                s.__year_zhi_index,
    ...
    fn get_time_position_xi(&self) -> String {
        POSITION_XI()[self.lock().unwrap().__time_gan_index as usize + 1].to_string()
    }
    ...
    fn get_time_position_yang_gui(&self) -> String {
        POSITION_YANG_GUI()[self.lock().unwrap().__time_gan_index as usize + 1].to_string()
    }
    ...
    fn get_time_position_yin_gui(&self) -> String {
        POSITION_YIN_GUI()[self.lock().unwrap().__time_gan_index as usize + 1].to_string()
    }
    ...
    fn get_time_position_fu(&self, sect: Option<i64>) -> String {
        ...
        position_fu()[self.lock().unwrap().__time_gan_index as usize + 1].to_string()
    }
    ...
    fn get_time_position_cai(&self) -> String {
        POSITION_CAI()[self.lock().unwrap().__time_gan_index as usize + 1].to_string()
    }
    ...
    fn get_day_chong_gan(&self) -> String {
        CHONG_GAN()[self.lock().unwrap().__day_gan_index as usize].to_string()
    }

    fn get_time_chong_gan(&self) -> String {
        CHONG_GAN()[self.lock().unwrap().__time_gan_index as usize].to_string()
    }
    ...
    fn get_day_chong_gan_tie(&self) -> String {
        CHONG_GAN_TIE()[self.lock().unwrap().__day_gan_index as usize].to_string()
    }

    fn get_time_chong_gan_tie(&self) -> String {
        CHONG_GAN_TIE()[self.lock().unwrap().__time_gan_index as usize].to_string()
    }
    ...
    fn get_festivals(&self) -> Vec<String> {
        ...
        if __month.abs() == 12 && __day >= 29 && __year != self.next(1).get_year() {
            fs.push(format!("除夕"));
        }
        fs
    }
    ...
    fn get_other_festivals(&self) -> Vec<String> {
        ...
        if __jie_qi.get("清明").unwrap().next(-1, None).to_ymd() == solar_ymd {
            fs.push(format!("寒食节"));
        }
    ...
    fn get_day_tian_shen(&self) -> String {
        ...
        TIAN_SHEN()[(__day_zhi_index + ZHI_TIAN_SHEN_OFFSET().get(&self.get_month_zhi()).unwrap())
            as usize
            % 12
            + 1]
        .to_string()
    ...
    fn get_day_position_tai(&self) -> String {
        POSITION_TAI_DAY()[LunarUtil::get_jia_zi_index(&self.get_day_in_gan_zhi()) as usize]
            .to_string()
    }
    ...
    fn get_day_ji_shen(&self) -> Vec<String> {
        LunarUtil::get_day_ji_shen(self.get_month(), &self.get_day_in_gan_zhi())
    }
    ...
    fn get_day_xiong_sha(&self) -> Vec<String> {
        LunarUtil::get_day_xiong_sha(self.get_month(), &self.get_day_in_gan_zhi())
    }
    ...
    fn get_day_nine_star(&self) -> NineStarRef {
        ...
        let dong_zhi_index =
            LunarUtil::get_jia_zi_index(&dong_zhi.get_lunar().get_day_in_gan_zhi());
        let dong_zhi_index2 =
            LunarUtil::get_jia_zi_index(&dong_zhi2.get_lunar().get_day_in_gan_zhi());
        let xia_zhi_index = LunarUtil::get_jia_zi_index(&xia_zhi.get_lunar().get_day_in_gan_zhi());
        ...
        let offset = match solar_date {
            ...
            d if d >= solar_shun_bai_date2 => self.get_solar().subtract(solar_shun_bai2) % 9,
            d if d < solar_shun_bai_date => (8 + solar_shun_bai.subtract(self.get_solar())) % 9,
            _ => 0,
        };
    ...
    fn get_time_nine_star(&self) -> NineStarRef {
        ...
        let asc = if jie_qi.get("冬至").unwrap().to_naivedate() <= solar_date
            && solar_date < jie_qi.get("夏至").unwrap().to_naivedate()
        {
            true
        } else if solar_date >= jie_qi.get("DONG_ZHI").unwrap().to_naivedate() {
            true
        } else {
            false
        };
    ...
    /// 节气: String
    ///
    fn get_next_jie_qi(&self, whole_day: Option<bool>) -> Option<JieQiRef> {
        let whole_day = whole_day.map_or(false, |f| f);
    ...
    /// 节气: String
    ///
    fn get_prev_jie_qi(&self, whole_day: Option<bool>) -> Option<JieQiRef> {
        let whole_day = whole_day.unwrap_or(false);
    ...
    fn get_jie_qi(&self) -> String {
        ...
        for (name, solar) in __jie_qi.iter() {
            if solar.get_year() == year && solar.get_month() == month && solar.get_day() == day {
    ...
    fn get_current_jie_qi(&self) -> Option<JieQiRef> {
        ...
        for (name, solar) in __jie_qi.iter() {
            if solar.get_year() == year && solar.get_month() == month && solar.get_day() == day {
                return Some(JieQi::new(&Lunar::__convert_jie_qi(name.as_str()), solar));
            }
        }
        None
    }
    ...
    fn get_current_jie(&self) -> Option<JieQiRef> {
        ...
        for key in JIE_QI_IN_USE().iter().step_by(2) {
            ...
            if d.get_year() == year && d.get_month() == month && d.get_day() == day {
    ...
    fn get_shu_jiu(&self) -> Option<ShuJiuRef> {
        ...
        start = solar::from_ymd(start.get_year(), start.get_month(), start.get_day());
        if current.is_before(start.clone()) {
            start = __jie_qi.get("冬至").unwrap().clone();
            start = solar::from_ymd(start.get_year(), start.get_month(), start.get_day());
        }
        let end =
            solar::from_ymd(start.get_year(), start.get_month(), start.get_day()).next(81, None);
    ...
    fn get_fu(&self) -> Option<FuRef> {
        ...
        let xia_zhi = { self.lock().unwrap().__jie_qi.get("夏至").unwrap().clone() };
        let li_qiu = { self.lock().unwrap().__jie_qi.get("立秋").unwrap().clone() };
    ...
    fn get_liu_yao(&self) -> String {
        LIU_YAO()
            [({ self.lock().unwrap().__month } + { self.lock().unwrap().__day } - 2) as usize % 6]
            .to_string()
    }
    ...
    fn get_wu_hou(&self) -> String {
        ...
        let mut index = { self.lock().unwrap().__solar.clone() }.subtract(jie_qi.get_solar()) / 5;
        if index > 2 {
            index = 2;
        }
    ...
    fn get_hou(&self) -> String {
        ...
        let mut offset = ({ self.lock().unwrap().__solar.clone() }.subtract(jie_qi.get_solar())
            as f64
            / 5.) as i64;
        if offset > size {
            offset = size;
    ...
    fn get_times(&self) -> Vec<LunarTimeRef> {
        ...
        let mut times = [LunarTime::from_ymd_hms(year, month, day, 0, 0, 0)].to_vec();
        for i in 0..12 {
