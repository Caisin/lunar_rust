use super::{JIE_QI_IN_USE, Lunar, LunarRef, LunarRefHelper, helper_traits::__LunarRefHelper};
use crate::{
...
impl __LunarRefHelper for LunarRef {
    fn __get_year_nine_star(&self, year_in_gan_zhi: &str) -> NineStarRef {
        ...
        let index = LunarUtil::get_jia_zi_index(&self.get_year_in_gan_zhi()) + 1;
        ...
        let yuan = ((self.lock().unwrap().__year + year_offset + 2696) as f64 / 60.) as i64 % 3;
    ...
    fn __get_near_jie_qi(
        ...
        let datetime_match_whole_day = |solar_ref: &SolarRef| -> NaiveDateTime {
            match whole_day {
                true => solar_ref.to_naivedate().and_hms_opt(0, 0, 0).unwrap(),
        ...
        for jq_key in JIE_QI_IN_USE().iter() {
            ...
            if forward {
                ...
                if near.is_none() {
                    ...
                    let near_day = datetime_match_whole_day(&near.clone().unwrap());
                    if day < near_day {
                        name = Some(jq.clone());
                        near = Some(solar.clone());
                    }
