use crate::{
    ...
    util::{
        lunar_util::{
            GAN, LunarUtil, NUMBER, POSITION_CAI, POSITION_DESC, POSITION_FU, POSITION_FU_2,
            POSITION_TAI_SUI_YEAR, POSITION_XI, POSITION_YANG_GUI, POSITION_YIN_GUI, ZHI,
        },
...
impl LunarYearRefHelper for Arc<Mutex<LunarYear>> {
    ...
    fn get_yuan(&self) -> String {
        format!(
            "{}{}",
            YUAN()[(({ self.lock().unwrap().__year } + 2696) as f64 / 60.) as usize % 3],
            "元"
        )
    }
    ...
    fn get_yun(&self) -> String {
        format!(
            "{}{}",
            YUN()[(({ self.lock().unwrap().__year } + 2696) as f64 / 20.) as usize % 9],
            "运"
        )
    }
    ...
    fn get_nine_star(&self) -> Arc<Mutex<NineStar>> {
        ...
        let yuan = (({ self.lock().unwrap().__year } + 2696) as f64 / 60.) as i64 % 3;
    ...
    fn get_position_xi(&self) -> String {
        POSITION_XI()[self.lock().unwrap().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_yang_gui(&self) -> String {
        POSITION_YANG_GUI()[self.lock().unwrap().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_yin_gui(&self) -> String {
        POSITION_YIN_GUI()[self.lock().unwrap().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_fu(self, sect: Option<i64>) -> String {
        ...
        position_fu()[self.lock().unwrap().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_cai(self) -> String {
        POSITION_CAI()[self.lock().unwrap().__gan_index as usize + 1].to_string()
    }
    ...
    fn get_position_tai_sui(self) -> String {
        POSITION_TAI_SUI_YEAR()[self.lock().unwrap().__zhi_index as usize].to_string()
    }
    ...
    ///
    fn next(&self, n: i64) -> Arc<Mutex<LunarYear>> {
        let year = { self.lock().unwrap().__year };
        LunarYear::from_lunar_year(year + n)
    }
}
...
impl LunarYearRefPrivateHelper for Arc<Mutex<LunarYear>> {
    fn __get_zao_by_gan(&self, index: i64, name: &str) -> String {
        let mut offset = index
            - solar::from_julian_day(self.get_month(1).unwrap().get_first_julian_day())
                .get_lunar()
                .get_day_gan_index();
        if offset < 0 {
            offset = offset + 10;
...
impl LunarYear {
    pub fn from_year(lunar_year: i64) -> Arc<Mutex<Self>> {
        Self::from_lunar_year(lunar_year)
    }

    pub fn compute(lunar_year: i64) -> Arc<Mutex<Self>> {
        Self::from_lunar_year(lunar_year)
    }
    ...
    fn __from_cache(lunar_year: i64) -> Option<Arc<Mutex<Self>>> {
        ...
        let found = cached_lunaryears
            .iter()
            .find(|lunar_year_arc| lunar_year == lunar_year_arc.lock().unwrap().__year);
        match found {
            Some(f) => Some(f.clone()),
            _ => None,
        }
    ...
    fn __compute(lunar_year: i64) -> Self {
        ...
        let mut jd = ((current_year - 2000) as f64 * 365.2422 + 180.).floor();
        // 355是2000.12冬至，得到较靠近jd的冬至估计值
        let mut w = ((jd - 355. + 183.) / 365.2422).floor() * 365.2422 + 355.;
        ...
        for i in 0..jie_qi_in_use.len() {
            if i == 0 {
                ...
                jd = ShouXingUtil::qi_accurate2(jq[25] + 15.2184 * (i - 26) as f64);
            }
        ...
        let leap_index = if __LEAP_11().iter().find(|y| **y == current_year).is_some() {
            // leap_index = 13;
            13
        ...
        for i in 0..15 {
            ...
            out_months.push(lunar_month::new(y, mc, day_counts[i], dm as f64, index));
            index += 1;
        }
...
__static_funk!(
    ...
    Vec<i64>,
    vec![
        75, 94, 170, 265, 322, 398, 469, 553, 583, 610, 678, 735, 754, 773, 849, 887, 936, 1050,
        1069, 1126, 1145, 1164, 1183, 1259, 1278, 1308, 1373, 1403, 1441, 1460, 1498, 1555, 1593,
        1612, 1631, 1642, 2033, 2128, 2147, 2242, 2614, 2728, 2910, 3062, 3244, 3339, 3616, 3711,
        3730, 3825, 4007, 4159, 4197, 4322, 4341, 4379, 4417, 4531, 4599, 4694, 4713, 4789, 4808,
        4971, 5085, 5104, 5161, 5180, 5199, 5294, 5305, 5476, 5677, 5696, 5772, 5791, 5848, 5886,
        6049, 6068, 6144, 6163, 6258, 6402, 6440, 6497, 6516, 6630, 6641, 6660, 6679, 6736, 6774,
        6850, 6869, 6899, 6918, 6994, 7013, 7032, 7051, 7070, 7089, 7108, 7127, 7146, 7222, 7271,
        7290, 7309, 7366, 7385, 7404, 7442, 7461, 7480, 7491, 7499, 7594, 7624, 7643, 7662, 7681,
        7719, 7738, 7814, 7863, 7882, 7901, 7939, 7958, 7977, 7996, 8034, 8053, 8072, 8091, 8121,
        8159, 8186, 8216, 8235, 8254, 8273, 8311, 8330, 8341, 8349, 8368, 8444, 8463, 8474, 8493,
        8531, 8569, 8588, 8626, 8664, 8683, 8694, 8702, 8713, 8721, 8751, 8789, 8808, 8816, 8827,
        8846, 8884, 8903, 8922, 8941, 8971, 9036, 9066, 9085, 9104, 9123, 9142, 9161, 9180, 9199,
        9218, 9256, 9294, 9313, 9324, 9343, 9362, 9381, 9419, 9438, 9476, 9514, 9533, 9544, 9552,
        9563, 9571, 9582, 9601, 9639, 9658, 9666, 9677, 9696, 9734, 9753, 9772, 9791, 9802, 9821,
...
__static_funk!(
    ...
    Vec<i64>,
    vec![
        37, 56, 113, 132, 151, 189, 208, 227, 246, 284, 303, 341, 360, 379, 417, 436, 458, 477,
        496, 515, 534, 572, 591, 629, 648, 667, 697, 716, 792, 811, 830, 868, 906, 925, 944, 963,
        982, 1001, 1020, 1039, 1058, 1088, 1153, 1202, 1221, 1240, 1297, 1335, 1392, 1411, 1422,
        1430, 1517, 1525, 1536, 1574, 3358, 3472, 3806, 3988, 4751, 4941, 5066, 5123, 5275, 5343,
        5438, 5457, 5495, 5533, 5552, 5715, 5810, 5829, 5905, 5924, 6421, 6535, 6793, 6812, 6888,
        6907, 7002, 7184, 7260, 7279, 7374, 7556, 7746, 7757, 7776, 7833, 7852, 7871, 7966, 8015,
        8110, 8129, 8148, 8224, 8243, 8338, 8406, 8425, 8482, 8501, 8520, 8558, 8596, 8607, 8615,
        8645, 8740, 8778, 8835, 8865, 8930, 8960, 8979, 8998, 9017, 9055, 9074, 9093, 9112, 9150,
        9188, 9237, 9275, 9332, 9351, 9370, 9408, 9427, 9446, 9457, 9465, 9495, 9560, 9590, 9628,
        9647, 9685, 9715, 9742, 9780, 9810, 9818, 9829, 9848, 9867, 9905, 9924, 9943, 9962, 10000
    ]
);
