...
use crate::{
    ...
    lunar::{self, LunarRef},
    solar_month::{SolarMonth, SolarMonthRefHelper},
    util::{
        holiday_util::{self, HolidayUtilRefHelper},
...
impl RefHelper for SolarRef {
    ...
    fn get_other_festivals(&self) -> Vec<String> {
        ...
        let _ = solar_util::get()
            .OTHER_FESTIVAL()
            .get(&(month, day))
            .map_or((), |v| festivals.append(&mut v.clone()));
        festivals
    }
    ...
    fn get_julian_day(&self) -> f64 {
        ...
        let mut y = year;
        let mut m = month;
        let d = day as f64 + ((second as f64 / 60.0 + minute as f64) / 60. + hour as f64) / 24.;
        let mut n = 0;
        ...
        let v = ((365.25 * (y as f64 + 4716.)) as i64 + (30.6001 * (m as f64 + 1.)) as i64) as f64
            + d as f64
            + n as f64
            - 1524.5;
    ...
    fn to_full_string(&self) -> String {
        ...
        self.clone()
            .get_festivals()
            .iter()
            .for_each(|f| s = format!("{s} ({f})"));
        self.get_other_festivals()
            .iter()
            .for_each(|f| s = format!("{s} ({f})"));
    ...
    fn is_after(&self, solar: Self) -> bool {
        if self.get_year() > solar.get_year() {
            return true;
        }
        if self.get_year() < solar.get_year() {
            return false;
        ...
        if self.get_day() > solar.get_day() {
            return true;
        }
        if self.get_day() < solar.get_day() {
            return false;
        }
        if self.get_hour() > solar.get_hour() {
            return true;
        }
        if self.get_hour() < solar.get_hour() {
            return false;
        }
        if self.get_minute() > solar.get_minute() {
            return true;
        }
        if self.get_minute() < solar.get_minute() {
            return false;
