...
impl Yun {
    pub fn new(eight_char: EightCharRef, gender: i64, sect: Option<i64>) -> YunRef {
        ...
        let (__start_year, __start_month, __start_day, __start_hour) = match sect {
            2 => {
            ...
            _ => {
                let end_time_zhi_index = match end.get_hour() == 23 {
                    true => 11,
                    _ => LunarUtil::get_time_zhi_index(&end.to_ymdhms().slice_by_char_idx(11, 16)),
                };
                let start_time_zhi_index = match start.get_hour() == 23 {
                    true => 11,
                    _ => {
                        LunarUtil::get_time_zhi_index(&start.to_ymdhms().slice_by_char_idx(11, 16))
                    }
                };
