...
use crate::{
    ...
    util::{
        ...
        lunar_util::{BASE_MONTH_ZHI_INDEX, GAN, LunarUtil, MONTH, ZHI},
    },
};
...
impl RefHelper for LiuYueRef {
    ...
    fn get_gan_zhi(&self) -> String {
        ...
        let offset = year_gan_to_offset.get(&year_gan).unwrap_or(&0).clone();
        let gan = &GAN()[(index + offset) as usize % 10 + 1];
        let zhi = &ZHI()[(index + BASE_MONTH_ZHI_INDEX()) as usize % 12 + 1];
        format!("{}{}", gan, zhi)
    }
