...
impl RefHelper for SolarWeekRef {
    ...
    fn get_index(&self) -> i64 {
        let mut offset =
            solar::from_ymd(self.get_year(), self.get_month(), 1).get_week() - self.get_start();
        if offset < 0 {
            offset = offset + 7;
        }
    ...
    fn get_index_in_year(&self) -> i64 {
        ...
        ((solar_util::get().get_days_in_year(self.get_year(), self.get_month(), self.get_day())
            + offset) as f64
            / 7.)
            .ceil() as i64
    }
            ...
            while 0 != n {
                ...
                if month != week_month {
                    ...
                    match plus {
                        ...
                        _ => {
                            match solar_util::get().get_weeks_of_month(
                                week.get_year(),
                                week.get_month(),
                                self.get_start(),
                            ) == index
                            {
                                ...
                                _ => {
                                    solar = solar::from_ymd(
                                        ...
                                        week.get_month(),
                                        solar_util::get()
                                            .get_days_of_month(week.get_year(), week.get_month()),
                                    );
                                    week = get_solar_week_from_solar(&solar);
                                }
                            };
