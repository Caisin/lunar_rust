use crate::{
    ...
    util::{
        ...
        lunar_util::{
            BASE_MONTH_ZHI_INDEX, GAN, MONTH, POSITION_CAI, POSITION_DESC, POSITION_FU,
            POSITION_FU_2, POSITION_GAN, POSITION_XI, POSITION_YANG_GUI, POSITION_YIN_GUI, ZHI,
        },
    },
};
...
impl LunarMonthRefHelper for LunarMonthRef {
    ...
    fn get_gan_index(&self) -> i64 {
        let offset = (LunarYear::from_lunar_year(self.get_year()).get_gan_index() + 1) % 5 * 2;
    ...
    fn get_nine_star(&self) -> Arc<Mutex<NineStar>> {
        let index = LunarYear::from_lunar_year(self.get_year()).get_zhi_index() % 3;
