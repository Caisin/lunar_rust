use crate::{
    ...
    solar_week::{SolarWeek, SolarWeekRefHelper},
...
fn test() {
    ...
    assert_eq!(
        5,
        solar_util::get().get_weeks_of_month(week.get_year(), week.get_month(), start)
    );
    // # 当周第一天
    ...
    assert_eq!(
        "2019-05-01",
        week.get_first_day_in_month().unwrap().to_string()
    );
}
