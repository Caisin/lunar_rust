...
#[test]
fn lunar_year_api_aliases_test() {
    let year = LunarYear::from_year(2023);
    assert_eq!(year.get_leap_month(), year.get_leap_months());
    assert_eq!(year.get_tou_liang(), year.get_tuo_liang());
    assert_eq!(year.get_cao_zi(), year.get_cai_zi());

    let next = year.next(1);
    assert_eq!(2024, next.get_year());
}
