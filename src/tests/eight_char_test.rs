use crate::{
    eight_char::{EightCharRefHelper, yun::YunRefHelper},
...
fn eight_char_test8() {
    ...
    let eight_char: std::sync::Arc<std::sync::Mutex<crate::eight_char::EightChar>> =
        lunar.get_eight_char();
    assert_eq!("壬寅", &eight_char.get_year());
    assert_eq!("戊申", &eight_char.get_month());
    assert_eq!("癸丑", &eight_char.get_day());
    assert_eq!("癸丑", &eight_char.get_time());
...
fn eight_char_test14() {
    ...
    let actual = solar_list
        .iter()
        .map(|s| s.to_naivedatetime())
        .collect::<Vec<_>>();
    let expected = vec![
        NaiveDate::from_ymd_opt(1999, 7, 21)
            .unwrap()
            .and_hms_opt(16, 0, 0)
            .unwrap(),
        NaiveDate::from_ymd_opt(1939, 8, 5)
            .unwrap()
            .and_hms_opt(16, 0, 0)
            .unwrap(),
    ];
...
fn eight_char_test15() {
    ...
    let actual = solar_list
        ...
        .collect::<Vec<_>>();
    let expected = vec![
        NaiveDate::from_ymd_opt(1960, 12, 17)
            .unwrap()
            .and_hms_opt(12, 0, 0)
            .unwrap(),
...
fn eight_char_test16() {
    ...
    let actual = solar_list
        ...
        .collect::<Vec<_>>();
    let expected = vec![
        NaiveDate::from_ymd_opt(2023, 2, 24)
            .unwrap()
            .and_hms_opt(23, 0, 0)
            .unwrap(),
        NaiveDate::from_ymd_opt(1843, 2, 8)
            .unwrap()
            .and_hms_opt(23, 0, 0)
            .unwrap(),
    ];
...
fn eight_char_test17() {
    ...
    let actual = solar_list
        ...
        .collect::<Vec<_>>();
    let expected = vec![
        NaiveDate::from_ymd_opt(1960, 1, 15)
            .unwrap()
            .and_hms_opt(16, 0, 0)
            .unwrap(),
        NaiveDate::from_ymd_opt(1900, 1, 29)
            .unwrap()
            .and_hms_opt(16, 0, 0)
            .unwrap(),
    ];
...
fn eight_char_test18() {
    ...
    let actual = solar_list
        ...
        .collect::<Vec<_>>();
    let expected = vec![
        NaiveDate::from_ymd_opt(1959, 12, 17)
            .unwrap()
            .and_hms_opt(16, 0, 0)
            .unwrap(),
    ];
