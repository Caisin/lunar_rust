use crate::{
    ...
    util::solar_util::{self, SolarUtilRefHelper},
};
...
fn test() {
    ...
    assert_eq!(
        ...
        solar.get_lunar().to_full_string()
    );
}
...
fn test13() {
    assert_eq!(
        "1582-09-30",
        solar::from_ymd(1582, 10, 15).next(-5, None).to_ymd()
    );
}
...
fn test14() {
    assert_eq!(
        "1582-10-04",
        solar::from_ymd(1582, 10, 15).next(-1, None).to_ymd()
    );
}
...
fn test15() {
    assert_eq!(
        "1582-09-29",
        solar::from_ymd(1582, 10, 15).next(-6, None).to_ymd()
    );
}
...
fn test19() {
    assert_eq!(
        "2023-09-30",
        solar::from_ymd(2023, 8, 31).next_month(1).to_ymd()
    );
}
...
fn test20() {
    assert_eq!(
        "2023-10-31",
        solar::from_ymd(2023, 8, 31).next_month(2).to_ymd()
    );
}
...
fn test21() {
    assert_eq!(
        "2024-02-29",
        solar::from_ymd(2023, 8, 31).next_month(6).to_ymd()
    );
}
...
fn test22() {
    assert_eq!(
        "2025-08-31",
        solar::from_ymd(2023, 8, 31).next_year(2).to_ymd()
    );
}

#[test]
fn solar_api_aliases_test() {
    assert_eq!(
        solar::from_ymdhms(2020, 5, 24, 13, 0, 0).to_ymdhms(),
        solar::from_ymd_hms(2020, 5, 24, 13, 0, 0).to_ymd_hms()
    );

    let list1 = solar::from_bazi("己卯", "辛未", "甲戌", "癸酉", Some(2), Some(1999));
    let list2 = solar::from_ba_zi("己卯", "辛未", "甲戌", "癸酉", Some(2), Some(1999));
